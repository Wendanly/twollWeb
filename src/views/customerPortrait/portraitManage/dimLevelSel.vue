        ...
        <div class="dialog-content">
            <div class="left">
                <div class="query-list">
                    <div class="form_item">
                        <el-input v-model="queryParam.dimValueDesc" placeholder="请输入维值名称" :clearable="true">
                            <span slot="append" class="query-icon" @click="clickQuery()">
                                <i v-iconfont="'iconsousuo-7'"></i>
                            </span>
                        </el-input>
                    </div>
                </div>
                <div class="table-list">
                    <sf-table ref="sfTable" :current="current" :pageSize="pageSize" :columns="columns" :data="data"
                        :total="total" :fixedTable="true" :selection="selection" @on-change="onchange($event)"
                        v-model="selectedItem" @select="tableSelect" @select-all="tableSelectAll"
                        @on-page-size-change="onpagesizechange($event)"></sf-table>
                </div>
            </div>
            <div class="right">
                ...
                <div class="sel-list">
                    <template v-if="dynamicTags!=null">
                        <el-tag :key="tag" v-for="tag in dynamicTags" :disable-transitions="false" closable
...
<script>
    ...
    export default {
        ...
        methods: {
            handleClose(tag) {
                ...
                for (let i = 0; i < this.data.length; i++) {
                    if (this.data[i].dimValue == tag.dimValue) {
                        this.$refs.sfTable.getElTable().toggleRowSelection(this.data[i], false);
                        break;
                    }
                }
            },
            ...
            query() {
                const tmp = {
                    dimLevelId: this.context.data.operand.dimLevelId,
                    type: "page",
                    page: this.current,
                    rows: this.pageSize,
                    dimValueDesc: this.queryParam.dimValueDesc
                };
            ...
            setSelectedItem() {
                ...
                for (let i = 0; i < this.data.length; i++) {
                    if (ids.indexOf(this.data[i].dimValue) > -1) {
                        this.$refs.sfTable.getElTable().toggleRowSelection(this.data[i], true);
                    }
                }
            }
        },
